// Netlify serverless function — DuoVino WSET mock-exam AI grader.
// The Anthropic API key lives ONLY here (ANTHROPIC_API_KEY env var, set via the
// Netlify CLI / dashboard); it is never shipped to the browser.
//
// The live site is served from GitHub Pages, so the browser calls this function
// CROSS-ORIGIN at https://duovino.netlify.app/.netlify/functions/grade — every
...
export default async (req) => {
  ...
  let body;
  try { body = await req.json(); } catch { return json({ error: "Bad request: invalid JSON" }, 400); }
  ...
  if (!process.env.ANTHROPIC_API_KEY) return json({ error: "Server not configured: ANTHROPIC_API_KEY is not set" }, 500);
