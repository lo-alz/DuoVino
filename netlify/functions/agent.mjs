...
// environment variable (set via the Netlify dashboard / CLI). It is never
...
export default async (req) => {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return new Response("Server not configured: ANTHROPIC_API_KEY is not set.", { status: 500 });
  }
  ...
  let mStream;
  try {
    mStream = client.messages.stream({
